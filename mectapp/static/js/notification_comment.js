...
$.ajax({
      ...
      if(parsedData.pdata[0].imgdata != 'NULL' && parsedData.pdata[0].postdata != 'NULL') {
        ...
        <div class="post-image" id="` + parsedData.pdata[0].postid +`">
         ...
         `+ (parsedData.cdata.length == 0 ? '' : '<div class="comment commentdiv"><img class="comment-img" src="/'+ parsedData.cdata[0].cdp +'"><h1 class="comment-text">'+ parsedData.cdata[0].commentdata +'</h1><h2 class="comment-see-all">Add a Comment</h2></div><div class="all-comments" id="'+ 0 +'"><div class="individual-comment-input"><img class="comment-img" src="/'+ userdp +'"><input type="text" placeholder="Your Comment" class="comment-input"></input><button class="comment-submit"><i class="fa fa-chevron-circle-right" aria hidden="true"></i></button></div></div></div>') +`
         `);
      }
